...
import { getcurrencyIcon, getMoneyText, getMoneyValue, round2, roundWithFormatAmount } from "./money-display";
...
export class SidebetUI {
    ...
    updateSideBetHistory(res) {
        ...
        if (Number(res.totalReward) > 0) {
            ...
            var balance = roundWithFormatAmount(res.totalReward);
            ...
            div = div + `<div class="flash-draw-second fund_prent">
                            ...
                            <div class="flash-bet-content">
                                ...
                                <div class="flash-bet-box">
                                    <button id="${option.betName}-${this.sidebetBB * 2}" class="betButton">${img} ${(roundWithFormatAmount(this.sidebetBB * 2 ))}</button>
                                    <button id="${option.betName}-${this.sidebetBB * 5}" class="betButton">${img} ${(roundWithFormatAmount(this.sidebetBB * 5))}</button>
                                    <button id="${option.betName}-${this.sidebetBB * 10}" class="betButton">${img} ${(roundWithFormatAmount(this.sidebetBB * 10))}</button>
                                </div>
                            </div>
                        </div>`;
        }
        ...
        for (const button of elements) {
            button.addEventListener('click', (e) => {
                ...
                if (e.currentTarget.classList.contains('selected')) {
                    ...
                    $(parentNode).find("#payout")[0].innerHTML = roundWithFormatAmount(payoutAmount);
                ...
                for (const payout of $(".text-yellow1")) {
                    if($(payout).find("#payout")[0].innerText){
                        let cleaned = $(payout).find("#payout")[0].innerText.replace(/,/g, '');
                        let num = parseFloat(cleaned);
                        totalPayout = totalPayout + Number(num);
                    }
                }
                ...
                $('#total-payout')[0].innerText = roundWithFormatAmount(totalPayout);
...
function updateTotalPaid(amount) {
    ...
    $('#total-paid')[0].innerHTML = roundWithFormatAmount(totalWinningAmount);
}

function updateFreeBalance(balance) {
    freeBalance = Number(Math.floor(balance).toFixed(1));
    var currency = getcurrencyIcon();
    $('#free-balance')[0].innerHTML = roundWithFormatAmount(freeBalance);
