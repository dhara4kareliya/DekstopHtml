...
import tableUi, { modes, Table } from "./table-ui";
import { getCurrentTurn, getPlayerSeat, myTotalMoneyInGame } from '../services/table-server';
...
for (const button of fourColorMenu) {
    button.addEventListener('click', () => {
        ...
        // $("#selectedDeckColor")[0].className = "color-indicator " + value;
        // document.body.className = value;
    });
}
...
function onTableSettings(settings) {
    ...
    if (settings.mode == "tournament") {
        ...
        mainUI.clearH4HTimer();
        mainUI.showUSDOption(false);

        // setShowDollarSign(false);
    } else {
        mainUI.showLevel(false);
        table.setSitVisible(true);
        mainUI.showUSDOption(true);
...
function onTableStatus(status) {
    ...
    if(mainPlayerSeat != -1){
        ...
        for (let index = 0; index < status.seats.length; index++) {
            let currentValue = status.seats[index];
            // if(currentValue.lastAction){

                if(mainPlayerSeat !== index ){
                    if (status.state !== 'PreFlop' || ( !['sb', 'bb'].includes(currentValue.lastAction) && currentValue.lastBet > 0)) {
                        isBet = true;
                    }
                    if(status.seats[mainPlayerSeat].lastBet < currentValue.lastBet  && !['fold','allin'].includes(status.seats[mainPlayerSeat].lastAction) && (['call', 'raise'].includes(currentValue.lastAction)) && status.state == prevRoundState ) {
                        isCallButton = true;
                        lastBet = Math.max(currentValue.lastBet,lastBet);
                    } else if(['sb'].includes(status.seats[mainPlayerSeat].lastAction) ){
                        isCallButton = true;
                        lastBet = tableSettings.bigBlind;
                    }
                // }
                if((status.seats[mainPlayerSeat].lastBet < currentValue.lastBet && ['call', 'raise'].includes(currentValue.lastAction)) || ['fold','allin', 'sb'].includes(status.seats[mainPlayerSeat].lastAction)) {
                    isCheckButton = false;
                }
            }
        }
        ...
        mainUI.setCheckFoldText(isCallButton);
    ...
    if (mainPlayerSeat != -1 && status.seats[mainPlayerSeat].state === 'Playing' && status.state == prevRoundState) {
        ...
        // mainUI.showautoCheckButton(status.seats[mainPlayerSeat].lastAction != 'sb' && !getCurrentTurn().canCheck);
        var isCheck = status.state != "Showdown" && ['sb', 'bb', undefined].includes(status.seats[mainPlayerSeat].lastAction) 
        mainUI.showFoldToAnyBetOption(isCheck && !isCallButton);
        mainUI.showautoCheckButton(isCheck && isCheckButton);
        mainUI.setCheckFold(isCheck);
    } else {
        mainUI.showFoldToAnyBetOption(false);
        mainUI.resetAutoCheckButton(false);  
    }
...
function onH4HMessage(res) {
    mainUI.showH4HMessage(res.status, res.msg);
}
...
tableSubscribe("onH4HMessage", onH4HMessage);
